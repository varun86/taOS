...
Last updated: 2026-06-14 ~02:20 BST, @taOS (active, autonomous overnight, 5h 24%).
...
Branch tips: master=6394a3ed. dev=15eaac00 (#871 store merged). Merged overall this session: #867 #868 #869 #870 (theme/wallpaper), #871 (store redesign); taos-website #5 (stats Basic Auth -> main, set STATS_USER/STATS_PASS in Coolify). Local-only `marketing` branch (private, no upstream; NEVER push/merge).

Session state: ACTIVE (autonomous overnight, 5h 24%). OPEN PRs baking (tests pending -> merge to dev when green): #872 (wallpaper sliders), #873 (real desktop), #874 (agent OS controls). #871 merged. tsParticles look + Safari dark<->light + live-wallpaper animation + desktop icons/thumbnails are all best checked LIVE on the Pi (preview has no backend; tsParticles canvas does not rasterize headless).
